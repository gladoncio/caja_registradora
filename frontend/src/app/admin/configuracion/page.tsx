...
import { Save, Print, Settings, Build, Router, Usb, Lock, People, Person, Money, ToggleOn } from '@mui/icons-material'
import api, { configAPI, impresoraAPI } from '@/lib/api'
...
export default function ConfigPage() {
  ...
  useEffect(() => {
    ...
    api.get('/monedas/').then(r => setMonedas(r.data)).catch(() => {})
  }, [])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (c.moneda_config) {
        setFormatConfig({
          moneda: {
            codigo: c.moneda_config.codigo,
            simbolo: c.moneda_config.simbolo,
            decimales: c.moneda_config.decimales,
            separador_miles: c.moneda_config.separador_miles,
            separador_decimal: c.moneda_config.separador_decimal,
            locale: c.moneda_config.locale,
          },
          redondeoMultiplo: c.redondeo_multiplo,
        })
      }
      localStorage.setItem('debug-mode', config.debug_mode ? 'true' : 'false')
      setTestMsg({ type: 'success', msg: 'Configuración guardada exitosamente' })
    }
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                    <FormControl fullWidth size="small">
                      ...
                      <Select value={config.moneda_principal || ''} label="Moneda principal"
                        onChange={(e) => setConfig({ ...config, moneda_principal: e.target.value as number })}
                        sx={{ borderRadius: 2 }}>
                        {monedas.filter((m: any) => m.activa).map((m: any) => (
                          <MenuItem key={m.id} value={m.id}>
                            <Box display="flex" gap={1} alignItems="center">
                              <Money fontSize="small" />
                              <Typography fontWeight={600}>{m.simbolo}</Typography>
                              <Typography variant="caption" color="text.secondary">{m.codigo} — {m.nombre}</Typography>
                            </Box>
                          </MenuItem>
                        ))}
                      </Select>
                  ...
                  <Grid item xs={6}>
                    <TextField fullWidth size="small" label="Múltiplo redondeo" type="number" value={config.redondeo_multiplo}
                      onChange={(e) => setConfig({ ...config, redondeo_multiplo: parseInt(e.target.value) || 0 })}
                      helperText="0 = sin redondeo"
                      sx={{ '& .MuiOutlinedInput-root': { borderRadius: 2 } }} />
                  </Grid>
                  <Grid item xs={6}>
                    <Box display="flex" alignItems="center" gap={1} height="100%" pt={1}>
                      <Button variant={config.debug_mode ? 'contained' : 'outlined'} color={config.debug_mode ? 'warning' : 'inherit'}
                        onClick={() => setConfig({ ...config, debug_mode: !config.debug_mode })}
                        startIcon={<ToggleOn />} sx={{ borderRadius: 2 }}>
                        Debug: {config.debug_mode ? 'ACTIVADO' : 'OFF'}
                      </Button>
                    </Box>
                  </Grid>
                  <Grid item xs={12}>
                    <Paper elevation={0} sx={{ p: 1.5, borderRadius: 2, bgcolor: (theme) => alpha(theme.palette.info.main, 0.04) }}>
                      <Box display="flex" justifyContent="space-between" alignItems="center">
                        <Typography variant="body2" color="text.secondary">
                          Administrar monedas y tasas de cambio
                        </Typography>
                        <Button size="small" variant="outlined" href="/monedas" sx={{ borderRadius: 2 }}>
                          Ir a Monedas
                        </Button>
                      </Box>
                    </Paper>
